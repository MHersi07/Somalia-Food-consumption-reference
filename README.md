...
SIHBS_reproduction_colab.ipynb    Same analysis as a Google Colab notebook
docs/METHODS_NOTES.md             Detail on the estimators
...
- **Solids only.** Table B1's factors for liquid items are denominated in grams
  rather than millilitres, implying a density the source does not state. Apply
  the table to solid items only — which achieves complete resolution for
  solids — and report liquid items from metric-unit records alone (97–99.9% of
  records), leaving the small non-metric remainder unconverted.
- **No meat entries.** Goat meat and camel meat have no entry, consistent with
  the empirical finding that they are reported exclusively in metric units.

Write the results into two columns, `resolved_grams` and `resolved_mL`, leaving
the other blank for each record.
...
The script verifies 47 published quantities: sample counts, household coverage
for all 11 headline items, per-capita quantities among consumers,
...
Alternatively, open `SIHBS_reproduction_colab.ipynb` in Google Colab and run
all cells; it prompts for the prepared file and needs no local installation.
...
**Variance.** Confidence intervals use Taylor linearisation of the Hájek ratio
estimator under stratified sampling, with region × residence as the design
strata (49 non-empty strata, minimum size 48). Clustering is not incorporated
because the released files omit the EA/PSU identifier; that omission is
parameterised through the design effect. See `docs/METHODS_NOTES.md`.

---

## Licence

The **code** is released under the MIT Licence (see `LICENSE`). Nothing else is
covered by it. This repository does not redistribute the SIHBS microdata, the
SIHBS unit codebook, the non-standard-unit conversion factors, or derived
